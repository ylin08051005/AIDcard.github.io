        ...
        for _ in range(10):  # adjust the range to control scrolling
            ...
            for img_elem in img_elements:
                ...
                if url and url.startswith('http'):
                     # 关键词过滤
                    if any(keyword in url.lower() for keyword in ["advertisement", "promo", "icon", "banner", "logo"]):
                        print(f"Skipped {url} due to keyword filtering")
                        continue
                    image_urls.add(url)
...
def is_valid_image(image):
    ...
    if aspect_ratio < 0.5 or aspect_ratio > 2:  # 假设物体的比例通常在0.5到2之间
        return False
